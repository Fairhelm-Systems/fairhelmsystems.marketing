...
Actual resource identifiers (account ID, bucket name, ACM certificate ARN, CloudFront distribution/OAC IDs, hosted zone IDs) are operational data, not documentation — they are not recorded in this repository. Ask the current maintainer for the live inventory, or discover them at deploy time: the script resolves everything it needs from tags and the active AWS CLI identity, and `docs/` deliberately stays free of account-specific values so this file can be public without disclosing production infrastructure.
...
SITE_BUCKET_NAME=fairhelmsystems-marketing-prod-<AWS_ACCOUNT_ID>
CLOUDFRONT_DISTRIBUTION_ID=
ACM_CERTIFICATE_ARN=
HOSTED_ZONE_ID_COM=<HOSTED_ZONE_ID_FOR_FAIRHELMSYSTEMS_COM>
HOSTED_ZONE_ID_IN=<HOSTED_ZONE_ID_FOR_FAIRHELMSYSTEMS_IN>
```
...
- Do not add real account IDs, bucket names, ARNs, distribution/OAC IDs, or hosted zone IDs to this file or any other tracked file — this repository is public.
