...
aws s3 sync "${OUT_DIR}/" "s3://${SITE_BUCKET_NAME}/" \
  ...
  --exclude "_next/*" \
  --exclude "*.md" \
  --exclude "insights/feed.xml" \
  --cache-control "public,max-age=0,must-revalidate" \
  --only-show-errors

# The Insights RSS feed (out/insights/feed.xml). Uploaded on its own so it is
# served as application/rss+xml rather than the CLI's generic application/xml
# guess, which is what feed readers and validators expect.
if [[ -f "${OUT_DIR}/insights/feed.xml" ]]; then
  log "Uploading the Insights RSS feed as application/rss+xml."
  aws s3 cp "${OUT_DIR}/insights/feed.xml" "s3://${SITE_BUCKET_NAME}/insights/feed.xml" \
